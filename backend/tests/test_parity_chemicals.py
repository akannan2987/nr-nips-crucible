...
def test_excel_upload_insert_and_update(client):
    ...
    # The identifier is generated by this application; DTX_ID is a separate
    # field carrying the value from the file. A compound whose file has no
    # DTX_ID gets an empty one rather than an invented identity.
    listing = client.get("/api/chemicals?limit=10").json()["data"]
    assert len(listing) == 1
    doc = listing[0]
    assert doc["chemical_id"] == "CHEM-000001"
    assert doc["dtx_id"] == "CHEM-E1"
