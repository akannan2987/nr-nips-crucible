...
[docs/07-operations.md → Database](07-operations.md#database-sqlite-and-postgresql).
...
| `structure` | object | No | v2.24.0: the one derived structure and its checks, written only by the derive step, never on upload; its picture (v2.25.0) is drawn on request and never stored — [`09-structures.md`](09-structures.md) | `{ "source": "smiles", "formula": "C8H10N4O2", "inchikey": "RYYVLZVUVIJVGH-UHFFFAOYSA-N", "checks": { ... }, "findings": [] }` |
