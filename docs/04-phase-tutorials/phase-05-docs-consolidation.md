...
**Version shipped:** 2.3.0 · **Date:** 2026-09-07 · **Status:** complete
...
| `docs/img/` and figures in every numbered doc | The ideas newcomers stumble on, drawn; generated by `docs/img/make_figures.py` | ✅ move 4 |
...
**How:** one script, [`docs/img/make_figures.py`](../img/make_figures.py),
writes every SVG — pure Python, deterministic bytes, one `symbol()` function
for the four record types. Each figure is placed under the heading that
explains its idea, with alt text that says what it shows; the
[figure index](../img/README.md) lists where each is used.

```bash
python3 docs/img/make_figures.py
git status --short docs/img/          # expect: no changes — the output is deterministic
```

**You should see:** eleven `wrote …` lines and a clean status.
...
tracked ignore list`; move 3 as `4a1d335 Add the phase tutorials, the product
roadmap and a Windows guide`; move 4, the figures and this page's completion,
is the v2.3.0 commit (all 2026-09-07). Each follows [`03-git-workflow.md` → Flow A](../03-git-workflow.md#4-flow-a---a-change-from-start-to-finish);
