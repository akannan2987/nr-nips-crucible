...
**First, decided 2026-09-08: a registry reset.** The identification logic is
about to change, and most registered compounds were created by the current
logic from the screening export. Rather than correct them one by one, the
registry starts again, in two deliberate steps, each behind a backup:

| Step | What | How | Waits on |
|---|---|---|---|
| R-1 | **Unlink every screening row from every chemical.** Rows keep the compound name their source file recorded; nothing else changes | a new `--unlink-all` option on `backend/scripts/remove_chemicals.py`, gated on `--apply`, run after `./container-py.sh backup` | the option, with its first pytest |
| R-2 | **Remove every chemical**, so the registry is empty and the new logic starts clean | a new `--all` option on the same script (unlink first, delete second), gated on `--apply`, after a second backup | R-1 and the owner's explicit go |
...
This gives the removal script the test it has never had (planned item below)
and closes the delete-endpoint question in passing, since after R-2 no row
points at anything.
...
| R | Registry reset 🔜 | Unlink every row, remove every chemical, then the new identification logic | the owner's go, then the owner's description of the new logic |
...
- **Re-registering the 22 removed compounds.** Superseded by the registry
  reset above: once the registry starts again under the new logic, these
  compounds are re-identified with everything else. Kept here so the history
  in [`11-lessons-learned.md`](11-lessons-learned.md) still has its pointer.
...
- **A test for the removal script.** It deletes data and has no automated
  test. *Waits on:* nothing — delivered with the registry reset's new options.
