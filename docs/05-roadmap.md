...
|---|---|---|---|
| CR | Registry CRUD and uploads (CSV, TSV, XLSX, SDF, JSON); one import module for every route, import/export scripts and shortcuts (CR-3 ✅ 2026-09-09); the three real sources as template specs (CR-9 ✅ 2026-09-09); three views, sort and per-column filters (CR-1 + CR-2 ✅ 2026-09-09); the attention page — every flag in the browser with merge, keep both, mark reviewed, set the identifier (CR-10 ✅ 2026-09-09); PubChem linking and enrichment scripts; audit, merge and removal scripts, now callers of the same modules as the browser; R-1 and R-2 done: the registry is empty by design (2026-09-08); CR-6 deletion refuses or forces ✅ (2026-09-09) | **CR-11 · counts, batch filters and source tags**, then **CR-12 · structures** (derive, draw, edit) — the owner's request of 2026-09-14, ahead of everything else; then the SD-1 build | 📝 CR-11 specified · 📝 CR-12 specified |
...
| **CR-11** | **Counts, batch filters and source tags.** A strip above the registry table — *all compounds · one batch · several batches · batch rows* — each count a button that filters the table; **tags** derived from the data when an entry is read (*Dotmatics ID*, *Dotmatics export*, *SDF upload*, *Excel upload*, *JSON upload*, *API*, later *Screening data*), shown as chips on the rows and in the detail, filterable one at a time or in combination (all of them, or any); `GET /api/chemicals/summary`, `batches=` and `tags=`/`tags_match=` on the list — [specification](09-registry-sources.md#counting-and-tagging-the-sources--specification) | 12,539 and 12,561 are on the same page with nothing between them explaining the difference, and an entry does not say where it came from; both facts are already in the data | decisions T1–T3 | 📝 specified 2026-09-14 · **next** |
