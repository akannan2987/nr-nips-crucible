...
|---|---|---|---|
| CR | Registry CRUD and uploads (CSV, TSV, XLSX, SDF, JSON); one import module for every route, import/export scripts and shortcuts (CR-3 ✅ 2026-09-09); the three real sources as template specs (CR-9 ✅ 2026-09-09); three views, sort and per-column filters (CR-1 + CR-2 ✅ 2026-09-09); the attention page — every flag in the browser with merge, keep both, mark reviewed, set the identifier (CR-10 ✅ 2026-09-09); the counts strip and the source tags (CR-11 ✅ 2026-09-14); one checked structure per entry, derived from what the source gave, the disagreements on the attention page (CR-12 step A ✅ 2026-09-23); every derived structure drawn on the server and shown wherever the compound appears (CR-12 step B ✅ 2026-09-23); PubChem linking and enrichment scripts; audit, merge and removal scripts, now callers of the same modules as the browser; R-1 and R-2 done: the registry is empty by design (2026-09-08); CR-6 deletion refuses or forces ✅ (2026-09-09) | **CR-12 · structures**: ~~A derive and check~~ ✅ v2.24.0; ~~B draw~~ ✅ v2.25.0; **C edit** (the editor, a save checked by RDKit, the history kept) next; then the SD-1 build | ✅ CR-11 v2.18.0 · 🔨 CR-12 A ✅ v2.24.0 · B ✅ v2.25.0 · C next |
...
| SH | Phases 00–05b: the Python backend, PostgreSQL option, public-repository hygiene, platform verification, the document set, reproducible builds and CI; SH-1 module names ✅ (2026-09-08); **SH-12 the beta instance ✅ (2026-09-21, v2.20.0; live on the server 2026-09-22)** — the scripts know their instance, the workflow has two moments; **SH-13 the instance label ✅ (2026-09-22, v2.21.0)**; **SH-3a the token gate ✅ (2026-09-22, v2.22.0)** — one shared token closes an instance's port; on beta and on production the same day, each with its own token; **SH-3b local accounts ✅ (2026-09-22, v2.23.0)** — usernames, passwords, roles and personal tokens, on beta first | the testers use their accounts on beta; production on `AUTH_MODE=local` since 2026-09-23 01:17 with its own accounts (v2.23.3, v2.23.4; [phase SH-3b, Step 10](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-10--production-in-two-moments-the-accounts-then-the-switch)); the login chapter closed; SH-3c single sign-on on hold (not needed for the moment, the owner, 2026-09-22); SH-4 roles in the page and the audit trail when the testers ask; **SH-2 · Schema normalisation** after | ✅ SH-3b v2.23.0 · ⏸ SH-3c · 🔜 SH-4 · 🔜 SH-2 |
...
| CR-8 | Merge a **hand-picked pair** from the registry table, with the confirmation the attention page uses | CR-10 merges flagged groups, where the candidates are already side by side; a duplicate the audit cannot see (two names for one substance, no shared identifier) needs a person to tick two rows | CR-10 ✅ | 🔜 small |
| CR-10 | **The attention page: every flag, in the browser.** The banner's counts link to one page listing the flagged entries — shared identifiers side by side with the other holders, batch conflicts with each batch's value, pending identifiers, the formula findings — with the actions a person takes: *merge*, *keep both*, *mark reviewed*, *set the identifier*, *delete*; a review mark on the entry; one audit module and one merge module behind the page, `GET /api/chemicals/audit` and three POST endpoints, and the two scripts as callers — [phase CR-10](04-phase-tutorials/phase-cr-10-attention-page.md) | The registry will be used by people who never open a terminal; a flag they cannot act on from the browser is a flag they will not act on. The first application of the rule below | — | ✅ v2.16.0 (2026-09-09) |
| CR-11 | **Counts, batch filters and source tags.** A strip above the registry table — *All compounds · One batch · Several batches · Batch rows* — each count a button that filters the table; six **tags** derived from the data when an entry is read (*Dotmatics ID, Excel upload, SDF upload, CSV upload, JSON upload, Manual*), chips on every row of every view and in the detail, filterable one at a time or in combination (all of them, or any); `GET /api/chemicals/summary`, `batches=` and `tags=`/`tags_match=` on the list, `registry_summary.py`; five tests — [phase CR-11](04-phase-tutorials/phase-cr-11-counts-and-tags.md) · [the rules and decisions](09-registry-sources.md#counting-and-tagging-the-sources) | 12,539 and 12,561 were on the same page with nothing between them explaining the difference, and an entry did not say where it came from; both facts were already in the data | — | ✅ v2.18.0 (2026-09-14) |
| **CR-12** | **Structures: derive, draw, edit**, in three releases. **A** — one canonical structure per entry computed by RDKit from the MOL block, SMILES or InChI the sources gave (6,553 of 12,539 have one), with computed formula, weight and InChIKey stored beside the laboratory's own values, and a *structure finding* on the attention page where they disagree; **B** — a 2-D picture drawn by the server (`GET /api/chemicals/{id}/structure.svg`) in the detail view, the chooser dialogs and an optional thumbnail column; **C** — a structure editor in the browser (Ketcher, Apache-2.0, bundled) to draw a structure for an entry with none or correct one, validated by RDKit on save, the previous structure kept — [specification](09-structures.md) | A structure is the one thing about a compound that can be checked against its formula and weight; the laboratory draws structures in other tools today and cannot in this one | decisions S1–S5 agreed 2026-09-23, S6–S9 taken while building; A before B before C | 🔨 **A ✅ v2.24.0 · B ✅ v2.25.0 (2026-09-23)** — [phase CR-12](04-phase-tutorials/phase-cr-12-structures.md) · C next |
...
endpoint. CR-10 (the attention page, v2.16.0) was the first application;
CR-8 (merge a hand-picked pair) and SH-7 (export from every page) follow it.
...
| SH-13 | **Prod and Beta, said in the page.** Every page shows which instance it is: an indigo *Prod* pill on a white bar, an amber *Beta* pill on an amber bar, the word on the tab; derived from the same `CRUCIBLE_INSTANCE` the scripts use, passed into the container, answered by one open endpoint `GET /api/instance`; five tests; the first change to travel publish-then-promote with a rebuild — [phase SH-13](04-phase-tutorials/phase-sh-13-instance-label.md) | Testers with several tabs open must never mistake production for beta | SH-12 | ✅ v2.21.0 (2026-09-22) |
| SH-12 | **A beta instance for user testing.** A second, complete copy of the application on the same server — its own folder on the `beta` branch, its own container and image (`crucible-py-beta`), port 49161, its own database refreshed from production's backup on request (`restore` takes a folder); `CRUCIBLE_INSTANCE` read by `container-py.sh`, the setup script, the monitor and the uninstaller, each acting on its own folder's instance; a second service unit and monitor line; the workflow gains *publish to beta* and *promote to master* as two moments; rehearsed on the development machine, set up on the server from [`01-setup-rhel8.md` §8](01-setup-rhel8.md#8-a-second-instance-for-user-testing-beta) — [phase SH-12](04-phase-tutorials/phase-sh-12-beta-instance.md) · [specification](14-beta-instance.md) · [ADR 0002](adr/0002-beta-instance.md). | End users are to test the application, and the login is to be rehearsed, without one click reaching the real registry | decisions B1–B6 — taken as recommended 2026-09-21 | ✅ v2.20.0 (2026-09-21) |
...
| **SH-3a** | **Token gate** — the first rung of the authentication ladder (was phase 07, now three rungs, planned in [`13-authentication.md`](13-authentication.md)): one feature flag `AUTH_MODE`, one guard on every route, one open health route, a login page, `verify-deploy.sh --token`; a shared secret closes the open port | The largest gap; needs nothing from anyone else; tokens remain for scripts on every later rung | ~~SH-12~~ ✅, so that it lands on beta first — agreed 2026-09-08, reordered 2026-09-21 | ✅ v2.22.0 (2026-09-22), on beta and on production the same day, each with its own token — [phase SH-3a](04-phase-tutorials/phase-sh-3a-token-gate.md) |
| **SH-3b** | **Local accounts** — usernames, Argon2-hashed passwords, a signed sliding session cookie, three roles enforced by one rule, personal tokens for scripts, `manage_users.py` behind `./container-py.sh users`, a change-password dialog, a lockout; **in full** (A3 revisited 2026-09-21: testers need per-person logins and the registration has not arrived) | Gives *who*, which unlocks the audit trail and per-person revocation; one login per tester | ~~SH-3a~~ ✅ v2.22.0; on beta; promoted to production after the test | ✅ v2.23.0 (2026-09-22), on beta first — [phase SH-3b](04-phase-tutorials/phase-sh-3b-local-accounts.md); production on accounts 2026-09-23 (Step 10, v2.23.3 and v2.23.4) |
| **SH-3c** | **Single sign-on** — OpenID Connect against the organisation's identity provider, roles from a group claim; the destination | Leavers lose access the day they leave; no password held here | SH-3a; an application registration from the identity team — [ask for it now](13-authentication.md#what-to-ask-the-organisation-for-now); decisions A1, A5, A6, A8 | ⏸ on hold: no single sign-on for the moment (the owner, 2026-09-22) |
| SH-4 | Role-based access everywhere (buttons greyed out per role in the page, an accounts page for the admin), audit trail and version history (*who* on every record), rate limiting | Meaningless without identity; possible now that SH-3b says who | ~~SH-3b~~ ✅ v2.23.0; the testers' feedback | 🔜 |
...
| SH-9 | `container-py.sh` waits until the application answers before `start`, `restart`, `rebuild` and `restore` return | A probe in the first two seconds after a rebuild failed with `Connection reset by peer`, and a deploy step that returns before the app is up invites exactly that | — | ✅ v2.21.2 (2026-09-22) |
...
flowchart LR
    ...
    SD1s -.-> SH12["SH-12 beta instance ✅<br/>v2.20.0"] --> SH3a["SH-3a token gate ✅<br/>v2.22.0"] --> SH3b["SH-3b local accounts ✅<br/>v2.23.0, on beta first"] --> SH3c["SH-3c single sign-on<br/>on hold: not needed for now"]
```
...
**2026-09-22:** SH-3a shipped (v2.22.0), the token gate, on beta and then on
production the same evening, each with its own token (v2.22.1 records it
and fixes the monitor's port rule). The owner decided that single sign-on is not needed for the moment, so SH-3c is
on hold and SH-3b, local accounts, is next.

**2026-09-22, late:** SH-3b shipped (v2.23.0), local accounts: one login per
tester with a role, personal tokens for scripts, a change-password dialog,
on beta first; production moves to accounts when the operator has created
them ([phase SH-3b](04-phase-tutorials/phase-sh-3b-local-accounts.md)). The
ladder's first two rungs are built; the third waits on the owner. Next in
order: CR-12 structures, then the SD-1 build.
...
to the registry's users every day, both are specified with their decisions
listed, and neither changes what SD-1 will do. SD-1 follows them, then
CR-5 and CR-4 as before.
