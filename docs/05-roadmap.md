...
| **SH-3b** | **Local accounts** — usernames, Argon2-hashed passwords, a signed sliding session cookie, three roles enforced by one rule, personal tokens for scripts, `manage_users.py` behind `./container-py.sh users`, a change-password dialog, a lockout; **in full** (A3 revisited 2026-09-21: testers need per-person logins and the registration has not arrived) | Gives *who*, which unlocks the audit trail and per-person revocation; one login per tester | ~~SH-3a~~ ✅ v2.22.0; on beta; promoted to production after the test | ✅ v2.23.0 (2026-09-22), on beta first — [phase SH-3b](04-phase-tutorials/phase-sh-3b-local-accounts.md); production when its accounts exist |
