...
| **API** | The same actions as web requests, sent with `curl` from any machine that can reach the server. `-k` is needed on the server because the certificate names the full host, not `localhost`; on a development machine use `http://` and drop `-k` | Scripts, bulk work, anything you want repeatable |
| **Terminal** | The maintenance scripts that run *inside* the container and talk to the database directly: `./container-py.sh script <name.py> [arguments]` from the repository folder, on any platform (it finds podman or Docker for you; `./container-py.sh script` alone lists the scripts). On the development machine with the test environment they also run directly: `cd backend && .venv/bin/python scripts/<name.py> …` | Bulk maintenance: audit, merge, remove, reset. The scripts that change data (remove, merge) report first and write only with `--apply`; the audit only reports; the PubChem scripts write as they go, in batches |
...
|---|---|---|
| **Chemical Registry** → type part of a name, an identifier (`CHEM-000042`) or a CAS number in the search box → **Search**. Then: **Compact · Complete · Batches** to choose the view; click a heading to sort; in Complete and Batches a filter box under every heading; **Rows per page** 20 to 500; **Choose columns** in Complete lists every column with its coverage. The view and the columns are remembered in your browser. Above the toolbar (v2.18.0): the **counts strip** — *All compounds · One batch · Several batches · Batch rows* — each a button that filters the table, and the **tag chips** — *Dotmatics ID, Excel upload, SDF upload, CSV upload, JSON upload, Manual* — tick one or several (*all of these* / *any of these*) to see only the entries that came that way; every row shows its tags | `curl --noproxy '*' -sSk "https://localhost:49160/api/chemicals?search=phenol&limit=20&page=1&sort=molecular_weight&order=desc"` — add `view=batches` for one row per batch, `filters={"metadata.CAS_NO":"58-"}` (URL-encoded) for a per-column filter, `batches=several` or `tags=Excel%20upload,SDF%20upload&tags_match=any` to filter by batches or by where the entries came from; `GET /api/chemicals/columns` lists every column; `GET /api/chemicals/summary` the counts and the entries per tag | The read-only SQL console: **Query** page, or `POST /api/query` with `{"sql": "SELECT chemical_id, json_extract(doc,'$.name') FROM chemicals WHERE json_extract(doc,'$.name') LIKE '%phenol%'"}` — recipes in [`09-query-cookbook.md`](09-query-cookbook.md); `./container-py.sh script registry_summary.py --tag "SDF upload"` lists the entries behind a tag, `--batches several` the compounds with several batches |

**You should see** matching rows with identifier, name, CAS, formula and
weight, and each row's source tags as coloured chips; in **Complete**, every column the files carried; in **Batches**,
...
| Browser | API | Terminal |
|---|---|---|
| **Chemical Registry** → the **eye icon** at the end of the row → a panel with every field, the derived structure drawn by the server (since v2.25.0; the project's own viewer for a MOL block never derived), the derived facts beside the laboratory's, and the spreadsheet's extra columns; tick **Pictures** in the Compact view for a thumbnail per row | `curl --noproxy '*' -sSk https://localhost:49160/api/chemicals/CHEM-000042` | SQL console: `SELECT doc FROM chemicals WHERE chemical_id='CHEM-000042'` — the whole record is the `doc` ([the one design rule](02-architecture.md#the-one-design-rule-everything-else-follows-from)) |
...
| Browser | API | Terminal |
|---|---|---|
| **Chemical Registry → Needs attention → Doubtful structures**: the derive panel says how many carry a source, how many are derived and how many still to derive; **Derive structures…** shows the report (nothing written), **Apply** stores it; one card per finding with the picture, *the source says* beside *its own structure is* and the verdict of each check; **It is fine — mark reviewed**. On any compound, *View* shows the drawing and a *Derived structure* strip; **Pictures** in the Compact view and the *structure (picture)* column in Complete and Batches show a thumbnail per row; the link chooser and the merge confirmation show the picture of the compound being confirmed (v2.25.0) | `POST …/api/chemicals/structures/derive` with `{}` for the report, `{"apply": true}` to write, `{"chemical_ids": [...]}` for a subset; `GET …/api/chemicals/audit` lists the findings under `structures`; `GET …/api/chemicals/CHEM-…` shows the `structure`; `GET …/api/chemicals/CHEM-…/structure.svg?w=320&h=240` is the picture, saved with `-o` ([the cookbook](08-api-cookbook.md#deriving-and-checking-the-structures)) | `./container-py.sh script derive_structures.py` for the report, `--apply` to write, `--ids CHEM-… ` for a subset, `--json` for the endpoint's answer; `./container-py.sh script audit_chemicals.py` prints the findings with the rest; `./container-py.sh script draw_structure.py CHEM-… -o /app/data/name.svg` writes the picture into the folder's `data/` |
