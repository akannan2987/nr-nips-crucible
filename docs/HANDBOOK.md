...
| Is it up? | `curl --noproxy '*' -sS http://localhost:49160/api/stats` | `curl --noproxy '*' -sSk https://localhost:49160/api/stats`; or `./container-py.sh status` in the instance's folder, or `systemctl --user status container-crucible-py[-beta].service` — [`15-run-stop-status.md`](15-run-stop-status.md) |
