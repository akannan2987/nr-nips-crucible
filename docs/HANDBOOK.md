...
| Accounts: add a person, reset a password, disable a leaver, issue a token | `./container-py.sh users add <name> --role viewer\|editor\|admin` · `reset` · `disable` · `token` · `list`, in the instance's folder; the mode `local` with a `SESSION_SECRET`, then `stop` and `start` | [Security → Accounts](07-operations.md#accounts-add-a-person-reset-a-password-disable-a-leaver-issue-a-token) · [phase SH-3b, Step 9](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-9--turn-it-on-beta-first) |
...
4. ~~SH-12 the beta instance~~ done, v2.20.0, and live on the server since
   ...
   roles; production moves to accounts when the operator has created them
   ([Step 10](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-10--production-when-its-accounts-exist));
