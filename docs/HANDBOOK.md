...
| | |
|---|---|
| **Version** | 2.25.0 (2026-09-23, CR-12 step B: every derived structure drawn on the server, `GET /api/chemicals/{id}/structure.svg`, the picture in the detail view, as a thumbnail column, in the link and merge dialogs and on the finding cards; the image gains the four system libraries RDKit's drawing needs; 2.24.0 the same day, CR-12 step A: one structure per entry derived by RDKit from the MOL block, SMILES or InChI the source gave, stored beside the entry with the canonical SMILES, InChIKey, formula, weights and counts; four checks against the source's own formula, weight and InChI; the disagreements a fifth kind on the attention page, 446 on the real export; one endpoint, one script, one button; 2.23.4 the same day, production on accounts since 01:17 server time by Step 10b, the switch and every proof recorded, the browser confirmed, the state after the switch tested by every route, the minute of the switch drawn; 2.23.3 the same day, Step 10 of the local-accounts tutorial split into two moments and production's three accounts created by the first, its login still the token until the switch the owner announces; `status` lists the accounts that wait; one test; 2.23.2 the day before, a start-to-finish walk for a script with a personal token in the cookbook; beta recorded as on accounts since 19:58; 2.23.1 the same day, the `users` command no longer swallows the lines pasted after it; 2.23.0 the same day, local accounts: one login per person with a role, a signed sliding session, personal tokens for scripts, accounts managed from the terminal; on beta first; 2.22.1 the same day, the login on production too, each instance with its own token; the monitor ignores a generic PORT; 2.22.0 the same day, the token gate: the first rung of the login, on beta first; 2.21.4 the same day, the development machine named by its role in every document, figure and comment; 2.21.3 the same day, the six blocks of every change written down; 2.21.2 the same day, one supervisor: the service runs the application and the script hands over; the run-stop-status guide; 2.21.1 the same day, the script keeps the systemd unit current; 2.21.0 the same day, the instance label: *Prod* and *Beta* in the page; 2.20.0 to 2.20.2 the day before and the same morning: the beta instance built, corrected, live); the first tagged release was `v2.10.1` |
| **Status date** | 2026-09-23 |
| **Tests** | 206 passing (`cd backend && .venv/bin/pytest`) |
| **Last phase done** | **CR-12 step B — structures drawn ✅ (2026-09-23, v2.25.0):** one drawing engine on the server, `GET /api/chemicals/{id}/structure.svg` (a MOL block as drawn, a SMILES laid out by CoordGen; an ETag from `derived_at`; nothing stored); the picture in the detail view, a *Pictures* toggle in Compact (off by default, S4) and a column in Complete and Batches, in the link chooser, the merge confirmation, the shared groups and the finding cards; `draw_structure.py`; five tests; the image gains `libxrender1`, `libx11-6`, `libxext6` and `libexpat1` (lesson 44) — [Part B](04-phase-tutorials/phase-cr-12-structures.md#part-b--draw-it). Before it, **CR-12 step A — structures derived and checked ✅ (2026-09-23, v2.24.0):** for every entry with a MOL block, a SMILES or an InChI, one derived structure under `structure`, never over the laboratory's fields: canonical SMILES, InChI and InChIKey, computed formula, average weight and exact mass, the counts, the source, the repair, the verdict of four checks; a disagreement is a *structure finding*, the fifth kind on the attention page with the two values side by side; `POST /api/chemicals/structures/derive`, `derive_structures.py` and **Derive structures…** call one module, report first, write on apply; four derived columns; the derived structure on the detail view; on the real export 6,553 with a source, 6,544 derived, 446 findings; eight tests; steps B (draw) and C (edit) next — [phase CR-12](04-phase-tutorials/phase-cr-12-structures.md) · [the reference](09-structures.md). Before it, **SH-3b — local accounts ✅ (2026-09-22, v2.23.0):** with `AUTH_MODE=local` and a `SESSION_SECRET` an instance asks for a username and a password; each account has a role, viewer, editor or admin, enforced by one rule from the verb and the path (403 names the role needed); the browser is remembered by a signed cookie that slides for ten hours from the last request; a personal token per account lets a script in and is revoked by name; `./container-py.sh users add · reset · role · enable · disable · unlock · token · remove · list` inside the container; a *Change password* dialog; ten wrong passwords lock the account; the users table holds hashes and the query console refuses it; twenty-three tests; delivered to beta first (2026-09-22 19:58), then production (2026-09-23 01:17 server time, Step 10 in two moments: the accounts, then the switch); **both instances on accounts, each with its own secret, the shared token retired on both** — [phase SH-3b](04-phase-tutorials/phase-sh-3b-local-accounts.md). Before it, **SH-3a — the token gate ✅ (2026-09-22, v2.22.0):** two lines in an instance's `.env.local` close its port; every module's routes answer 401 without the token and as before with it; a login page with the instance pill, a cookie that is a keyed hash of the token, a *Sign out*; `GET /api/health` open for the probes; `verify-deploy.sh --token`; the cross-origin policy closed; on beta at 17:31 and on production at 18:06 the same day, each with its own token (decision A11) — [phase SH-3a](04-phase-tutorials/phase-sh-3a-token-gate.md). Before it, **SH-13 — the instance label ✅ (2026-09-22, v2.21.0):** every page says *Prod* (indigo) or *Beta* (amber) in a pill and on the tab, derived from the same instance name the scripts use, through one open endpoint `GET /api/instance`; the first change to travel publish-then-promote with a rebuild — [phase SH-13](04-phase-tutorials/phase-sh-13-instance-label.md). Before it, **SH-12 — a beta instance for user testing ✅ (2026-09-21, v2.20.0, live on the server 2026-09-22):** a second, complete copy of the application beside production, named by one file (`CRUCIBLE_INSTANCE=beta`, `CRUCIBLE_PORT=49161`) that every script reads; its own container, image, data, unit and monitor line; `restore` takes a folder so beta is refreshed from production in two commands; the workflow now has two moments, publish to beta and promote to master; rehearsed on the development machine with two containers side by side; the server setup is [`01-setup-rhel8.md` §8](01-setup-rhel8.md#8-a-second-instance-for-user-testing-beta) — [phase SH-12](04-phase-tutorials/phase-sh-12-beta-instance.md). Before it, CR-11 — counts, batch filters and source tags ✅ (2026-09-14): the strip of batch counts as filter buttons, six source tags derived from the data as chips in every view, filterable in combination, one summary endpoint and script — [phase CR-11](04-phase-tutorials/phase-cr-11-counts-and-tags.md). Before it, CR-10 — the attention page ✅ (2026-09-09): every flag in the browser with the buttons to act — merge, keep both, mark reviewed, set the identifier — behind one audit module the page, the API and the script all call; a review mark stored on the entry — [phase CR-10](04-phase-tutorials/phase-cr-10-attention-page.md). Before it the same day, CR-2 with CR-1 — three views, sort and filter on the registry table ✅: every column (164), every batch (12,561), sorted and filtered on the server — [phase CR-2](04-phase-tutorials/phase-cr-2-views-sort-filter.md); and CR-9 — The real registry sources ✅: the Dotmatics export, the structure file and the limited list recognised by their columns and loaded by their own rules; batches folded, sources merged on DTXSID, shared identifiers flagged, structures read; a banner and the audit for what a person decides — [phase CR-9](04-phase-tutorials/phase-cr-9-real-registry-sources.md) · [the sources](09-registry-sources.md) |
...
| **Plan** | Six **tracks**, one per module and a shared spine, each with its next phase — [`05-roadmap.md`](05-roadmap.md). Next in order: ~~the owner loads the three real files~~ done 2026-09-09 (12,539 entries on production) → ~~CR-10 the attention page~~ done v2.16.0 → ~~CR-11 counts and tags~~ done v2.18.0 → ~~SH-12 the beta instance~~ done v2.20.0, live on the server 2026-09-22 → ~~SH-13 the instance label~~ done v2.21.0 → ~~SH-3a the token gate~~ done v2.22.0, on beta first → ~~SH-3b local accounts~~ done v2.23.0, one login per tester on beta; production on accounts since 2026-09-23 01:17 ([Step 10, in two moments](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-10--production-in-two-moments-the-accounts-then-the-switch)); the login chapter is closed → CR-12 structures: ~~A derive and check~~ done v2.24.0, ~~B draw~~ done v2.25.0, C edit next ([phase CR-12](04-phase-tutorials/phase-cr-12-structures.md) · [the reference](09-structures.md)) → SD-1 build → CR-5 unregistered review → CR-4 incomplete entries → CR-8 merge a hand-picked pair → SH-2 schema normalisation; SH-3c single sign-on ⏸ not needed for the moment (the owner, 2026-09-22) |
| **Production** | one RHEL 8 VM, one container per instance — production on 49160 and, **since 2026-09-22, the beta instance on 49161** with its own copy of production's data (restored from the morning's backup), its own unit and monitor line, **both behind the login since 2026-09-22 (v2.22.0, each with its own token; beta on local accounts since 2026-09-22 19:58 with v2.23.0, one login per tester; production on accounts since 2026-09-23 01:17 with its own three accounts and its own secret; the shared token retired on both)** — one SQLite file each: 49,065 screening rows, **12,539 registered chemicals** (the Dotmatics export, the structure file and the limited list, loaded by the owner on 2026-09-09 through the terminal shortcut), **0 rows linked** until SD-1 attaches them; 419 entries share an identifier, 3 have batch conflicts and 125 have a formula their names do not explain — 795 items on the attention page, for a person to decide, of which 447 structure findings since the structures were derived on both instances on 2026-09-23 (v2.24.0, [Step 7](04-phase-tutorials/phase-cr-12-structures.md#step-7--derive-on-the-server-beta-first)); every derived structure drawn since v2.25.0; the 664 old entries stay in `~/data-backup-20260908-before-R2.db` on the server |
...
| 2026-09-22 | **v2.23.0.** SH-3b: local accounts — usernames, Argon2-hashed passwords, three roles enforced by one rule, a signed sliding session, personal tokens for scripts, accounts managed from the terminal, a change-password dialog, the lockout; on beta first, production when its accounts exist. |
| 2026-09-22 | **v2.23.1.** The `users` shortcut attaches the terminal's input to the container only for the two verbs that read a password; before, the lines pasted after it were swallowed (lesson 41). |
| 2026-09-22 | **v2.23.2.** Beta on accounts since 19:58, recorded; the cookbook walks a script through a personal token from start to finish, with a shell and a Python script. Documents only. |
| 2026-09-23 | **v2.23.3.** Production's accounts before its switch: Step 10 of the local-accounts tutorial is two moments, the accounts (created 2026-09-23, the login still the token) and the switch at a moment the owner announces; `status` on the token rung lists the accounts that wait; the state between the two tested by every route; one test (193). |
| 2026-09-23 | **v2.23.4.** Production on accounts since 01:17 server time: the switch (Step 10b) and every proof recorded, the browser confirmed; the state after the switch tested by every route; the minute of the switch drawn. Both instances use accounts. Documents only. |
| 2026-09-23 | **v2.24.0.** CR-12 step A: one checked structure per compound, derived by RDKit from what the source gave and stored beside the laboratory's values; four checks (formula against every fragment, weight against both masses, the InChI by layer, unreadable sources); a fifth kind on the attention page; endpoint, script and button on one module. 446 findings on the real export (447 on the server). |
| 2026-09-23 | **v2.25.0.** CR-12 step B: every derived structure drawn on the server as an SVG that follows the structure's version; the picture in the detail view, the table (a toggle, a column), the link and merge dialogs and the finding cards; the image gains the four system libraries RDKit's drawing needs (lesson 44). |
...
If the instance asks you to **sign in** first — an access token, or since
v2.23.0 a username and a password — use what its operator gave you, and on
a first visit change the password from the top bar —
[playbook → Signing in](10-user-playbook.md#signing-in). A fresh
installation has the login off.
...
| CR-12 | CR | Structures: derive, draw, edit | **A** (v2.24.0): one derived structure per entry from the MOL block, SMILES or InChI (S5), stored beside the entry (`structure`: canonical SMILES, InChI, InChIKey, formula, average weight, exact mass, counts, source, repair); four checks against the source's own formula (every fragment), weight (both masses) and InChI (by layer), and unreadable sources; the findings a fifth kind on the attention page; `POST /api/chemicals/structures/derive`, `derive_structures.py`, the Derive button; four derived columns; the detail view; eight tests. **B** (v2.25.0): `GET /api/chemicals/{id}/structure.svg` drawn by RDKit on the server (a MOL block as drawn, a SMILES by CoordGen; an ETag from `derived_at`); the picture in the detail view, a Pictures toggle and a column, the link and merge dialogs, the finding cards; `draw_structure.py`; five tests; the image's X11 libraries. **C** edit to come | [`phase-cr-12-structures.md`](04-phase-tutorials/phase-cr-12-structures.md) | 2026-09-23 (v2.24.0 A, v2.25.0 B) | 🔨 A ✅ · B ✅ · C next |
...
| SH-3b | SH | Local accounts | `AUTH_MODE=local` and `SESSION_SECRET`: a `users` table (migration 0002) with Argon2-hashed passwords; a signed cookie that slides for ten hours from the last request; three roles enforced by one rule from the verb and the path, 403 naming the role needed; personal tokens for scripts, revoked by name; `manage_users.py` behind `./container-py.sh users`; the login page's username-and-password form, who-am-I and *Change password* in the top bar; the lockout; the users table refused by the query console; `verify-deploy.sh` says who a token belongs to; twenty-three tests; on beta first (2026-09-22), production 2026-09-23 in two moments, the accounts then the switch (v2.23.3, v2.23.4) | [`phase-sh-3b-local-accounts.md`](04-phase-tutorials/phase-sh-3b-local-accounts.md) | 2026-09-22 (v2.23.0) | ✅ |
...
| 07 (SH-3a/b/c) | SH | Authentication, as a ladder (agreed 2026-09-08; reordered to the top 2026-09-21) | A token gate (SH-3a), local accounts in full (SH-3b), single sign-on through the organisation's identity provider (SH-3c) — one flag, one guard on every route, one open health route; planned in [`13-authentication.md`](13-authentication.md), decided in [ADR 0001](adr/0001-authentication-ladder.md) | [`phase-sh-3a-token-gate.md`](04-phase-tutorials/phase-sh-3a-token-gate.md) · [`phase-sh-3b-local-accounts.md`](04-phase-tutorials/phase-sh-3b-local-accounts.md) | 2026-09-22 (v2.22.0, v2.23.0) | ✅ SH-3a · ✅ SH-3b · ⏸ SH-3c on hold (the owner, 2026-09-22) |
...
| Accounts: add a person, reset a password, disable a leaver, issue a token | `./container-py.sh users add <name> --role viewer\|editor\|admin` · `reset` · `disable` · `token` · `list`, in the instance's folder; the mode `local` with a `SESSION_SECRET`, then `stop` and `start` | [Security → Accounts](07-operations.md#accounts-add-a-person-reset-a-password-disable-a-leaver-issue-a-token) · [phase SH-3b, Step 9](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-9--turn-it-on-beta-first) · on production, the accounts first and the switch at an announced moment: [Step 10](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-10--production-in-two-moments-the-accounts-then-the-switch) |
...
2026-09-22, each with its own token. The second rung, local accounts
(v2.23.0), says who is calling and with which role: one account per
person, managed from the terminal, on beta first and on production when its
accounts exist; single sign-on is on hold —
...
4. ~~SH-12 the beta instance~~ done, v2.20.0, and live on the server since
   ...
   accounts~~ done, v2.23.0: one login per tester on the beta instance, with
   roles; production on accounts since 2026-09-23 01:17 ([Step 10, in two
   moments](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-10--production-in-two-moments-the-accounts-then-the-switch));
   single sign-on (SH-3c) on hold, not needed for the moment; SH-4 (roles
   in the page, an accounts page, the audit trail) when the testers ask
...
5. ~~CR-11~~ done, v2.18.0. **CR-12** — the owner's request of 2026-09-14:
   structures derived, drawn and editable in the browser, three releases:
   ~~A derive and check~~ done, v2.24.0; ~~B draw~~ done, v2.25.0
   (2026-09-23; [phase CR-12](04-phase-tutorials/phase-cr-12-structures.md));
   **C edit** (the editor, Ketcher) next ([the reference](09-structures.md)).
...
9. **SH-3a, b, c — authentication, as a ladder:** the largest gap, now
   two-thirds closed. `/api/*` was open to anyone who could reach the port;
   the token gate (v2.22.0) closed it, local accounts (v2.23.0) say who and
   with which role; single sign-on stays the destination, on hold until the
   owner asks the identity team. The plan, every method explained, and what
   to ask for: [`13-authentication.md`](13-authentication.md).
...
| The login (accounts) | rehearse: `AUTH_MODE=local SESSION_SECRET=… ./container-py.sh rebuild`, then `./container-py.sh users add me --role admin` | `users add` one per person; `AUTH_MODE=local` and `SESSION_SECRET=…` in `.env.local`, `stop` then `start`; `users token <me>`, then `CRUCIBLE_TOKEN=<it> ./verify-deploy.sh …` → 19 passed — [phase SH-3b](04-phase-tutorials/phase-sh-3b-local-accounts.md#step-9--turn-it-on-beta-first) |
