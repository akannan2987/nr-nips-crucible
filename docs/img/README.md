...
| `fig_switch_minute.svg` | The minute of the switch: the settings file before and after, the five steps between (backup, stop, start, unit rewritten, answers), and what changes for a browser, a script, the monitor, the data and the accounts; the way back | `04-phase-tutorials/phase-sh-3b-local-accounts.md`, `07-operations.md` |
| `fig_structure_derive.svg` | CR-12 step A: the three text forms of a structure go into RDKit in a fixed order; one derived structure with computed facts is stored beside the entry; the four checks compare the source's own formula, weight and InChI with its structure; a disagreement is a finding | `04-phase-tutorials/phase-cr-12-structures.md`, `09-structures.md` |
| `fig_structure_draw.svg` | CR-12 step B: one drawing engine on the server turns the derived structure into an SVG shown in the detail view, the table, the dialogs and the findings, and saved by a script; the picture carries the structure's version; the coordinates rule | `04-phase-tutorials/phase-cr-12-structures.md`, `09-structures.md` |
